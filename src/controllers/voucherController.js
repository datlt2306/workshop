...
export const getAllVouchers = asyncHandler(async (req, res, next) => {
    ...
    const features = new APIFeatures(queryBuilder, req.query)
        .filter()
        .sort()
        .limitFields()
        .paginate();
...
export const getVoucher = asyncHandler(async (req, res, next) => {
    ...
    if (!voucher) {
        return next(new AppError("Không tìm thấy voucher với ID này", StatusCodes.NOT_FOUND));
    }
...
export const getVoucherByCode = asyncHandler(async (req, res, next) => {
    ...
    if (!voucher) {
        return next(
            new AppError("Không tìm thấy voucher hợp lệ với mã này", StatusCodes.NOT_FOUND)
        );
    }
...
export const createVoucher = asyncHandler(async (req, res, next) => {
    ...
    if (!code || !amount || !endDate) {
        return next(
            new AppError("Vui lòng cung cấp đầy đủ thông tin voucher", StatusCodes.BAD_REQUEST)
        );
    }
    ...
    if (existingVoucher) {
        return next(new AppError("Mã voucher đã tồn tại", StatusCodes.BAD_REQUEST));
    }
