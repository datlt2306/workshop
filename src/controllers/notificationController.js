...
export const createNotification = asyncHandler(async (req, res, next) => {
  ...
  if (!type) {
    return next(new AppError("Vui lòng cung cấp loại thông báo", 400))
  }
...
const getDefaultMessage = (type, orderNumber, total) => {
  switch (type) {
    case "order_created":
      return `Đơn hàng #${orderNumber} đã được tạo với tổng giá trị ${total?.toLocaleString("vi-VN")}₫`
    case "order_status_changed":
      return `Trạng thái đơn hàng #${orderNumber} đã được cập nhật`
    case "payment_received":
      return `Đã nhận thanh toán cho đơn hàng #${orderNumber} với số tiền ${total?.toLocaleString("vi-VN")}₫`
    default:
