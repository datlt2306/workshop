...
export const createOrder = asyncHandler(async (req, res) => {
  ...
  for (const item of cart.items) {
    ...
    if (currentStock < item.quantity) {
      throw new AppError(`Sản phẩm ${product.name} không đủ số lượng trong kho`, 400)
    }
  ...
  await Notification.create({
    ...
    message: `Đơn hàng của bạn đã được tạo thành công với tổng giá trị ${newOrder.total.toLocaleString("vi-VN")}₫`,
    data: {
  ...
  for (const admin of adminUsers) {
    await Notification.create({
      ...
      message: `Đơn hàng mới đã được tạo bởi ${req.user.name} với tổng giá trị ${newOrder.total.toLocaleString("vi-VN")}₫`,
      data: {
        orderId: newOrder._id,
        orderNumber: newOrder.orderNumber,
      },
...
export const updateOrderStatus = asyncHandler(async (req, res) => {
  ...
  if (!status) {
    throw new AppError("Vui lòng cung cấp trạng thái đơn hàng", 400)
  }
...
export const cancelOrder = asyncHandler(async (req, res) => {
  ...
  if (!["pending", "processing"].includes(order.status)) {
    throw new AppError("Đơn hàng này không thể hủy ở trạng thái hiện tại", 400)
  }
  ...
  for (const admin of adminUsers) {
    await Notification.create({
      ...
      message: `Đơn hàng #${order.orderNumber} đã bị hủy bởi khách hàng với lý do: ${req.body.cancelReason || "Không có lý do"}`,
      data: {
        orderId: order._id,
        orderNumber: order.orderNumber,
      },
...
export const createPaymentIntent = asyncHandler(async (req, res) => {
  ...
  if (order.status === "cancelled") {
    throw new AppError("Không thể thanh toán đơn hàng đã bị hủy", 400)
  }
...
export const handlePaymentSuccess = asyncHandler(async (req, res) => {
  ...
  await Notification.create({
    ...
    message: `Thanh toán cho đơn hàng #${order.orderNumber} đã được xác nhận với số tiền ${order.total.toLocaleString("vi-VN")}₫`,
    data: {
      orderId: order._id,
      orderNumber: order.orderNumber,
    },
...
export const getOrderStats = asyncHandler(async (req, res) => {
  ...
  if (isDateRangeStats) {
    ...
    if (!startDate || !endDate) {
      throw new AppError("Vui lòng cung cấp ngày bắt đầu và kết thúc", 400)
    }
